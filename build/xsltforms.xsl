...
<xsl:template name="html" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:exslt="http://exslt.org/common" xmlns:xalan="http://xml.apache.org/xalan">
	...
	<html>
		<xsl:copy-of select="@*"/>
		<xsl:comment>HTML elements and Javascript instructions generated by XSLTForms rev.629 (629) - Copyright (C) 2016 &lt;agenceXML&gt; - Alain COUTHURES - http://www.agencexml.com</xsl:comment>
		...
		<head>
			...
			<xsl:variable name="jsrevisionname"><xsl:if test="$script/config/properties/jsrevision = 'concat'">-rev.629</xsl:if></xsl:variable>
			<xsl:variable name="jsrevisionparam"><xsl:if test="$script/config/properties/jsrevision != '' and $script/config/properties/jsrevision != ''">?<xsl:value-of select="$script/config/properties/jsrevision"/>=629</xsl:if></xsl:variable>
			<script type="text/javascript" id="xsltforms-src" src="{$resourcesdirjs}xsltforms{$jsrevisionname}.js{$jsrevisionparam}" data-uri="http://www.agencexml.com/xsltforms">
				<xsl:attribute name="data-version">629</xsl:attribute>
				<xsl:text>/* */</xsl:text>
			</script>
