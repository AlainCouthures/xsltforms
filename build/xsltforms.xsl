...
<xsl:stylesheet xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:ajx="http://www.ajaxforms.net/2006/ajx" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:exslt="http://exslt.org/common" xmlns:txs="http://www.agencexml.com/txs" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" exclude-result-prefixes="xhtml xforms ev exslt msxsl"><rdf:RDF>
		<rdf:Description rdf:about="http://www.agencexml.com/xsltforms/xsltforms.xsl">
			...
			<dcterms:hasVersion>600</dcterms:hasVersion>
			<dcterms:creator>Alain Couthures - agenceXML</dcterms:creator>
			<dcterms:conformsTo>XForms 1.1</dcterms:conformsTo>
			<dcterms:created>2014-06-06</dcterms:created>
			<dcterms:description>Empty label fix</dcterms:description>
...
<html xmlns:xf="http://www.w3.org/2002/xforms" xmlns:xsltforms="http://www.agencexml.com/xsltforms">
	...
	<body>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xalan" name="html">
			...
			<html>
				<xsl:copy-of select="@*"/>
				<xsl:comment>HTML elements and Javascript instructions generated by XSLTForms r600 - Copyright (C) 2008-2014 &lt;agenceXML&gt; - Alain COUTHURES - http://www.agencexml.com</xsl:comment>
				...
				<head>
					...
					<script id="xsltforms-src" src="{$resourcesdir}xsltforms.js" type="text/javascript" data-uri="http://www.agencexml.com/xsltforms">
						<xsl:attribute name="data-version">600</xsl:attribute>
						<xsl:text>/* */</xsl:text>
					</script>
			...
			<xsl:choose>
				<xsl:when test="namespace-uri(parent::*) = 'http://www.w3.org/2000/svg'">
					<xsl:element name="tspan" namespace="http://www.w3.org/2000/svg">
						...
						<xsl:if test="local-name() != 'trigger' and local-name() != 'submit' and local-name() != 'reset' and (string(xforms:label) != '' or xforms:label/@*) and string($appearance) != 'none'">
							<xsl:choose>
								<xsl:when test="$appearance = 'minimal'">
									<xsl:apply-templates select="node()">
										<xsl:with-param name="parentworkid" select="$workid"/>
				...
				<xsl:otherwise>
					<span>
						...
						<xsl:if test="local-name() != 'trigger' and local-name() != 'submit' and local-name() != 'reset' and local-name() != 'output' and (string(xforms:label) != '' or xforms:label/@*) and string($appearance) != 'none'">
						...
						<xsl:if test="local-name() != 'output' and $required-position = 'left' and (string(xforms:label) != '' or xforms:label/@*)">
						...
						<xsl:if test="local-name() != 'output' and local-name() != 'component' and not($required-position = 'left' and (string(xforms:label) != '' or xforms:label/@*))">
							<span class="xforms-required-icon">*</span>
						</xsl:if>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" name="toXPathExpr">
			...
			<xsl:call-template name="escapeJS">
				...
				<xsl:with-param name="trtext" select="translate($p,concat($carriagereturn,$tab,'&quot;'),concat($newline,$newline,$newline))"/>
			</xsl:call-template>
		</xsl:template>
	  ...
	  <xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" name="xpath">
					...
					<xsl:when test="$c = &quot;'&quot;">
						...
						<xsl:variable name="t2">
							...
							<xsl:call-template name="escapeJS">
								...
								<xsl:with-param name="trtext" select="translate($t,concat($carriagereturn,$tab,'&quot;'),concat($newline,$newline,$newline))"/>
							</xsl:call-template>
							<xsl:text>'</xsl:text>
						</xsl:variable>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" name="escapeJS">
			...
			<xsl:choose>
				<xsl:when test="contains($trtext, $newline)">
					<xsl:value-of select="substring-before($trtext, $newline)"/>
					<xsl:variable name="c" select="substring($text, string-length(substring-before($trtext, $newline))+1, 1)"/>
					<xsl:choose>
						<!--xsl:when test="$c = ' '"><xsl:text> </xsl:text></xsl:when-->
					...
					<xsl:call-template name="escapeJS">
						...
						<xsl:with-param name="trtext" select="substring-after($trtext, $newline)"/>
					</xsl:call-template>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" match="xforms:component" mode="script" priority="1">
			...
			<js xmlns="">
				...
				<xsl:text>",</xsl:text>
				<xsl:choose>
					<xsl:when test="string(xforms:label) != '' or xforms:label/@*">2</xsl:when>
					<xsl:otherwise>0</xsl:otherwise>
				</xsl:choose>
				<xsl:text>,</xsl:text>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" match="xforms:input | xforms:secret | xforms:textarea" mode="script" priority="1">
			...
			<js xmlns="">
				...
				<xsl:text>",</xsl:text>
				<xsl:choose>
					<xsl:when test="(string(xforms:label) != '' or xforms:label/@*) and $required-position = 'left'">3</xsl:when>
					<xsl:when test="string(xforms:label) != '' or xforms:label/@*">2</xsl:when>
					<xsl:otherwise>0</xsl:otherwise>
				</xsl:choose>
				<xsl:text>,"</xsl:text>
						...
						<xsl:choose>
							<xsl:when test="@mediatype and @mediatype != 'application/xml'">
								...
								<xsl:call-template name="escapeJS">
									...
									<xsl:with-param name="trtext" select="translate(.,concat($newline,$carriagereturn,$tab,'\&quot;'),concat($newline,$newline,$newline,$newline,$newline))"/>
								</xsl:call-template>
								<xsl:text>"</xsl:text>
							</xsl:when>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" match="xforms:text" mode="itext" priority="1">
			...
			<xsl:call-template name="escapeJS">
				...
				<xsl:with-param name="trtext" select="translate(xforms:value,concat($newline,$newline,$newline,'&quot;'),concat($newline,$newline,$newline,$newline))"/>
			</xsl:call-template>
			<xsl:text>"</xsl:text>
			<xsl:if test="following-sibling::*">
				<xsl:text>,</xsl:text>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" match="xforms:output | xforms:label[(@ref or @bind) and not(parent::xforms:item) and not(parent::xforms:itemset) and not(parent::xforms:choices)]" mode="script" priority="1">
			...
			<js xmlns="">
				...
				<xsl:text>",</xsl:text>
				<xsl:choose>
					<xsl:when test="string(xforms:label) != '' or xforms:label/@*">1</xsl:when>
					<xsl:otherwise>0</xsl:otherwise>
				</xsl:choose>
				<xsl:text>,</xsl:text>
				<xsl:call-template name="toScriptBinding"><xsl:with-param name="p" select="@ref"/></xsl:call-template>
