...
<xsl:stylesheet xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:ajx="http://www.ajaxforms.net/2006/ajx" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:exslt="http://exslt.org/common" xmlns:txs="http://www.agencexml.com/txs" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" exclude-result-prefixes="xhtml xforms ev exslt msxsl"><rdf:RDF>
		<rdf:Description rdf:about="http://www.agencexml.com/xsltforms/xsltforms.xsl">
			...
			<dcterms:hasVersion>623</dcterms:hasVersion>
			<dcterms:creator>Alain Couthures - agenceXML</dcterms:creator>
			<dcterms:conformsTo>XForms 1.1</dcterms:conformsTo>
			<dcterms:created>2015-10-07</dcterms:created>
			<dcterms:description>Workaround for Firefox running scripts twice</dcterms:description>
...
<html xmlns:xf="http://www.w3.org/2002/xforms" xmlns:xsltforms="http://www.agencexml.com/xsltforms">
	...
	<body>
		...
		<xsl:template xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xalan" name="html">
			...
			<html>
				<xsl:copy-of select="@*"/>
				<xsl:comment>HTML elements and Javascript instructions generated by XSLTForms r623 - Copyright (C) 2008-2015 &lt;agenceXML&gt; - Alain COUTHURES - http://www.agencexml.com</xsl:comment>
				...
				<head>
					...
					<script type="text/javascript" id="xsltforms-initialdate">
						...
						<xsl:if test="$script/config/properties/domengine">
							<xsl:value-of select="$script/config/properties/domengine/@uri"/>
						</xsl:if>
						<xsl:text>";
</xsl:text>
					...
					<xsl:if test="$script/config/properties/domengine">
						<script type="text/javascript" id="{$script/config/properties/domengine/@name}-src" src="{$script/config/properties/domengine/@url}" data-uri="{$script/config/properties/domengine/@uri}" data-version="{$script/config/properties/domengine/@version}">
							<xsl:text>/* */</xsl:text>
						</script>
					</xsl:if>
					...
					<xsl:variable name="jsrevision"><xsl:if test="$script/config/properties/jsrevision != ''">?<xsl:value-of select="$script/config/properties/jsrevision"/>=623</xsl:if></xsl:variable>
					<script type="text/javascript" id="xsltforms-src" src="{$resourcesdir}xsltforms.js{$jsrevision}" data-uri="http://www.agencexml.com/xsltforms">
						<xsl:attribute name="data-version">623</xsl:attribute>
						<xsl:text>/* */</xsl:text>
					</script>
